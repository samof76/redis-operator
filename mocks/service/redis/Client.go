...
// MakeSlaveOf provides a mock function with given fields: ip, port, masterIP, password
func (_m *Client) MakeSlaveOf(ip string, port string, masterIP string, password string) error {
	ret := _m.Called(ip, port, masterIP, password)

	var r0 error
	if rf, ok := ret.Get(0).(func(string, string, string, string) error); ok {
		r0 = rf(ip, port, masterIP, password)
	} else {
		r0 = ret.Error(0)
	}
